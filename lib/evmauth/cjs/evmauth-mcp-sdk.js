...
class EVMAuthSDK {
    ...
    normalizeServerProof(serverProof) {
        ...
        if (typeof serverProof.message === 'string') {
            try {
                ...
                throw new errors_1.EVMAuthError('PROOF_MALFORMED', `Failed to parse server proof message: ${error.message}`);
            }
        }
    ...
    extractWalletFromJWT(payload) {
        ...
        if (payload.wallet_address && (0, viem_1.isAddress)(payload.wallet_address)) {
            return payload.wallet_address.toLowerCase();
        }
        // Fallback to sub claim (legacy format)
        if (payload.sub && (0, viem_1.isAddress)(payload.sub)) {
            return payload.sub.toLowerCase();
        }
        // If neither exists or are invalid addresses
        throw new errors_1.EVMAuthError('JWT_INVALID', 'JWT missing valid wallet address in wallet_address or sub claim');
    }
    /**
    ...
    async verifyProof(proof) {
        ...
        if (Date.now() > proof.expiresAt) {
            throw new errors_1.EVMAuthError('PROOF_EXPIRED', 'Proof has expired');
        }
        ...
        if (proofChainId !== this.config.chainId) {
            throw new errors_1.EVMAuthError('CHAIN_MISMATCH', `Chain ID mismatch: expected ${this.config.chainId}, got ${proofChainId}`);
        }
        // Verify domain matches our configuration
        const { domain } = proof.challenge;
        if (domain.name !== 'EVMAuth' || domain.version !== '1') {
            throw new errors_1.EVMAuthError('DOMAIN_MISMATCH', 'Invalid domain name or version');
        }
        if (domain.chainId !== this.config.chainId) {
            throw new errors_1.EVMAuthError('DOMAIN_MISMATCH', `Domain chain ID mismatch: expected ${this.config.chainId}, got ${domain.chainId}`);
        }
        if (domain.verifyingContract.toLowerCase() !== this.config.contractAddress.toLowerCase()) {
            throw new errors_1.EVMAuthError('DOMAIN_MISMATCH', `Domain contract mismatch: expected ${this.config.contractAddress}, got ${domain.verifyingContract}`);
        }
        try {
            // 1. Verify EIP-712 signature and recover signer address
            const recoveredAddress = await (0, viem_1.recoverTypedDataAddress)({
                domain: {
                    name: domain.name,
            ...
            try {
                ...
                if (jwtError.name === 'TokenExpiredError') {
                    throw new errors_1.EVMAuthError('JWT_EXPIRED', 'JWT token has expired');
                }
                throw new errors_1.EVMAuthError('JWT_INVALID', `JWT verification failed: ${jwtError.message}`);
            }
            ...
            if (this.config.jwtIssuer && jwtPayload.iss !== this.config.jwtIssuer) {
                throw new errors_1.EVMAuthError('JWT_INVALID', `Invalid JWT issuer: expected ${this.config.jwtIssuer}, got ${jwtPayload.iss}`);
            }
            ...
            if (this.config.expectedAudience) {
                ...
                if (!validAudience) {
                    throw new errors_1.EVMAuthError('JWT_INVALID', `JWT audience does not match expected: ${this.config.expectedAudience}`);
                }
            }
                ...
                throw new errors_1.EVMAuthError('JWT_INVALID', 'JWT audience does not match server name');
            }
            // 6. Verify JWT wallet matches recovered signer
            if (jwtWalletAddress !== recoveredAddress.toLowerCase()) {
                throw new errors_1.EVMAuthError('JWT_SUBJECT_MISMATCH', 'JWT wallet address does not match signature signer');
            }
            // 5. Additional JWT validations
            if (jwtPayload.evmauth) {
                if (jwtPayload.evmauth.chainId !== this.config.chainId) {
                    throw new errors_1.EVMAuthError('JWT_INVALID', 'JWT chain ID mismatch');
                }
                if (jwtPayload.evmauth.contractAddress?.toLowerCase() !== this.config.contractAddress.toLowerCase()) {
                    throw new errors_1.EVMAuthError('JWT_INVALID', 'JWT contract address mismatch');
                }
            }
            return recoveredAddress.toLowerCase();
        }
        catch (error) {
            if (error instanceof errors_1.EVMAuthError) {
                throw error;
            }
            throw new errors_1.EVMAuthError('SIGNATURE_INVALID', `Signature verification failed: ${error.message}`);
        }
    }
    /**
    ...
    async verifyDevProof(proof) {
        if (!this.config.devMode) {
            throw new errors_1.EVMAuthError('PROOF_INVALID', 'Dev mode proof provided but dev mode is not enabled');
        }
        const { walletAddress } = proof;
        // Validate wallet address format
        if (!walletAddress || !(0, viem_1.isAddress)(walletAddress)) {
            throw new errors_1.EVMAuthError('PROOF_INVALID', 'Invalid wallet address in dev proof');
        }
        // Log dev mode usage
    ...
    validateConfig(config) {
        ...
        if (!(0, viem_1.isAddress)(config.contractAddress)) {
            throw new errors_1.EVMAuthError('INVALID_CONFIG', 'Invalid contract address format');
        }
        // Validate chain ID
        if (!Number.isInteger(config.chainId) || config.chainId <= 0) {
            throw new errors_1.EVMAuthError('INVALID_CONFIG', 'Chain ID must be a positive integer');
        }
        // Validate RPC URL
        try {
            ...
            throw new errors_1.EVMAuthError('INVALID_CONFIG', 'Invalid RPC URL format');
        }
    }
    /**
     * Validate proof structure
    ...
    validateProofStructure(proof) {
        if (!proof || typeof proof !== 'object') {
            throw new errors_1.EVMAuthError('PROOF_INVALID', 'Invalid proof format');
        }
        const { challenge, signature, chainId, expiresAt } = proof;
        // Validate required fields
        if (!challenge || !signature || !expiresAt) {
            throw new errors_1.EVMAuthError('PROOF_INVALID', 'Missing required proof fields');
        }
        // chainId can be at top level or in domain
        const effectiveChainId = chainId || challenge.domain?.chainId;
        if (!effectiveChainId) {
            throw new errors_1.EVMAuthError('PROOF_INVALID', 'Missing chainId in proof');
        }
        // Validate signature format
        if (typeof signature !== 'string' || !signature.startsWith('0x')) {
            throw new errors_1.EVMAuthError('PROOF_INVALID', 'Invalid signature format');
        }
        // Validate challenge structure
        if (!challenge.domain || !challenge.types || !challenge.primaryType || !challenge.message) {
            throw new errors_1.EVMAuthError('PROOF_INVALID', 'Invalid challenge structure');
        }
        ...
        if (!message.serverName || !message.resourceName || !message.requiredTokens ||
            ...
            throw new errors_1.EVMAuthError('PROOF_INVALID', 'Challenge message missing required fields');
        }
    }
    /**
